<x-app-layout>
    ...
    <div>
        <form action="{{ route('category.update',$category->slug) }}" method="POST" enctype="multipart/form-data" class="mb-[80px]">
            @csrf
                ...
                <div class="h-auto w-full flex justify-evenly">
                    <div class="w-3/5 h-auto shadow-md p-[20px] flex flex-col items-center gap-y-[20px]">
                        <div class="w-full flex flex-col gap-y-3">
                            ...
                            <input type="text" name="name" id="" class="w-full rounded-md" value="{{ $category->name }}">
                            @error('title')
                         ...
                         <div class="w-full flex flex-col gap-y-2 mt-[10px]">
                            ...
                            <input type="text" value="{{ $category->meta_title }}" class="w-full rounded-md border-gray-300" name="meta_title"
                                id="">

                        </div>
                        <div class="w-full flex flex-col gap-y-2 mt-6">
                            <label for="" class="text-[20px] font-[500]">Meta Description</label>
                            <textarea name="meta_description" id="" cols="30" rows="6" class="w-full">{{ $category->meta_description }}</textarea>
