    ...
    <div class="md:p-8 bg-white rounded-lg shadow-xs h-auto">
        <div class="">
            ...
            <a href="{{ route('category.list') }}"
                class="inline-block float-right px-4 py-2 mb-[10px] text-sm font-medium text-white bg-green-600 rounded hover:bg-green-800">
                See List
            </a>
            <form action="{{ route('category.store') }}" method="post" class="w-full flex flex-col gap-y-5"
                enctype="multipart/form-data">
                @csrf
                <div class="flex items-center">
                    <label class="md:text-[18px] font-[500] md:w-[150px]">Category Name :</label>
                    <input type="text" name="name" id=""
                        class="md:w-[400px] outline-none rounded-md md:ml-[50px]" value="{{ old('name') }}">
                    @error('name')
